...
| v1.5.20260319-1 | 优化：悬停滚动按钮显示延迟调整为1秒，隐藏延迟调整为0.6秒，新增仅在tbody区域触发显示逻辑，滚轮滚动时检测鼠标位置 |
| v1.5.20260320-1 | 优化：离开 tbody 优先级最高，删除鼠标在按钮上的保护逻辑，滚轮滚动改用 tbody 区域判断 |
...
| 事件 | 行为 | 延迟 | 说明 |
|------|------|------|------|
| `mouseenter` 进入表格（仅 tbody 区域） | 启动显示计时器 | 1秒 | 悬停 1 秒后显示滚动按钮，**仅当鼠标在数据区域（tbody）内时触发** |
| `mouseleave` 离开表格 | 清除显示计时器 | - | 如果未满 1 秒，取消显示 |
...
| 事件 | 行为 | 延迟 | 说明 |
|------|------|------|------|
| `mouseleave` 离开表格 | 启动隐藏计时器 | 0.6秒 | 离开表格 0.6 秒后隐藏按钮 |
| `mousemove` 移动超过阈值 | 启动隐藏计时器 | 0.6秒 | 移动超过 25px 且不在按钮上时触发 |
| `wheel` 滚轮滚动 | 检测鼠标位置 | - | 滚轮滚动时若鼠标离开 tbody 区域则隐藏按钮 |

**注意**：离开 tbody 优先级最高，一旦离开 tbody 区域，无论鼠标是否在按钮上，都会开始 0.6 秒隐藏倒计时。
...
└─────────────┘                          └──────┬──────┘
       ...
       │                                 └──────┬──────┘
       │                                        │
       │ mouseleave / wheel离开tbody /          │ mouseleave /
       │ mousemove>25px不在按钮上               │ wheel离开tbody /
       │ (启动0.6s隐藏计时器)                    │ mousemove>25px不在按钮上
       │                                        ↓
       │                                 ┌─────────────┐
       │                                 │  等待隐藏    │
       │                                 │  (hiding)   │
       ...
       │                                        │
       │ 0.6秒到                                 │ mouseenter按钮
       │ (回到waiting)                          │ (keepButtons回到visible)
       │                                        ↓
       │                                 ┌─────────────┐
       └─────────────────────────────────│   显示状态   │
               再次进入tbody               │  (visible)  │
...
1. **仅 tbody 区域触发**：只有当鼠标在数据区域（tbody）内时才会触发按钮显示，在表头（thead）区域不会触发

2. **实时位置更新**：按钮显示后仍然跟随鼠标移动，确保鼠标移动到新位置时按钮在正确位置

3. **单计时器机制**：显示和隐藏共用一个 `buttonTimer`，状态转换时自动清除旧计时器

4. **移动阈值检测**：新增 25px 移动阈值，避免误触发隐藏（鼠标轻微抖动不会触发）

5. **防闪烁处理**：鼠标从表格移入按钮区域时，会清除隐藏计时器，确保按钮不会闪烁消失（仅在 hiding 状态下有效）

6. **滚动步长**：每次点击左右按钮滚动 120px

7. **离开 tbody 优先级最高**：一旦鼠标离开 tbody 区域，无论是否在按钮上，都会开始隐藏倒计时

8. **滚轮滚动检测**：滚轮滚动时检测鼠标位置是否离开 tbody 区域，若离开则隐藏按钮
...
let isOnButton = false;                     // 标记鼠标是否在按钮上（用于取消隐藏，仅 hiding 状态有效）
