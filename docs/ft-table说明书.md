...
| `wheel` 滚轮滚动 | 检测鼠标位置 | - | 滚轮滚动时若鼠标在表格外则隐藏按钮 |
...
5. **防闪烁处理**：鼠标从表格移入按钮区域时，会清除隐藏计时器，确保按钮不会闪烁消失

6. **滚动步长**：每次点击左右按钮滚动 120px

7. **显示时立即隐藏**：按钮显示状态下离开表格，立即启动 0.6 秒隐藏计时器

8. **滚轮滚动检测**：滚轮滚动时检测鼠标位置，若鼠标在表格外则隐藏按钮
